...
class CognitiveComplexityAnalyzer:
    ...
    def _analyze_function(self, file_path: Path, node: ast.FunctionDef, content: str) -> CodeComplexity:
    ...
    def _calculate_confusion_score(self, cyclomatic: int, indirection: int, 
                                 context_switches: int, imports: int) -> float:
...
class ConfusionReporter:
    ...
    def generate_report(self, threshold: float = 5.0, verbose: bool = False) -> Dict[str, Any]:
        ...
        report = {
            "summary": {
                ...
                "refactoring_priority": "high" if avg_confusion > 7 else "medium" if avg_confusion > 4 else "low"
            },
        ...
        if verbose:
            report["detailed_analysis"] = {
                ...
                "analysis_timestamp": "2025-09-01",  # Could use actual timestamp
                "all_hotspots": [asdict(h) for h in hotspots]
            }
            
        return report
    
    def run_analysis(self, threshold: float = 5.0, verbose: bool = False, 
                    output_file: Optional[str] = None, generate_plan: bool = False) -> int:
    ...
    def _generate_refactoring_plan(self, report: Dict[str, Any], output_file: str) -> None:
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"**Overall Confusion Score:** {report['summary']['overall_confusion_score']}\n")
...
def main():
    ...
    parser.add_argument("--threshold", type=float, default=5.0, 
                       help="Confusion score threshold for hotspots")
    parser.add_argument("--verbose", action="store_true", 
                       help="Include detailed analysis in output")
    parser.add_argument("--focus", type=str, 
                       help="Focus analysis on specific directory")
    parser.add_argument("--plan", action="store_true",
                       help="Generate refactoring plan instead of JSON")
    parser.add_argument("--output", type=str,
                       help="Output file path")
    parser.add_argument("--ci", action="store_true",
                       help="CI mode with exit codes")
    parser.add_argument("--max-confusion", type=float, default=7.0,
                       help="Maximum acceptable confusion score for CI")
