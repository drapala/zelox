...
class LLMReadinessChecker:
    ...
    def check_co_location(self) -> Tuple[int, str]:
        ...
        for feature_dir in feature_dirs:
            has_code = any((feature_dir / f).exists() for f in ["service.py", "models.py", "api.py"])
        ...
        return score, f"{status} Co-location: {co_located_count}/{total_count} features ({co_location_ratio:.1%})"
    ...
    def check_front_matter_coverage(self) -> Tuple[int, str]:
        ...
        for py_file in python_files:
            ...
            try:
                with open(py_file, 'r', encoding='utf-8') as f:
                    ...
                    if ('"""' in content and any(keyword in content.lower() for keyword in 
                        ['title:', 'purpose:', 'inputs:', 'outputs:'])) or content.startswith('---'):
                        files_with_frontmatter += 1
            except Exception:
                continue
                
        total_files = len([f for f in python_files if "test" not in f.name and "__init__" not in f.name])
        if total_files == 0:
        ...
        return score, f"{status} Front-matter coverage: {files_with_frontmatter}/{total_files} files ({coverage:.1%})"
